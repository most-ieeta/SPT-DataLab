...
public class SPTDataLab {
    ...
    public static TestBuilderModel model2() {
        return instance().tbModel2;
    }
    ...
    public void setClearScreen(boolean value) {
        clearScreen = value;
    }
...
private static void startCleanApp() {
    ...
    /**
     * Open prompt for user to load dataset (image, then geometry files) then
     * load to memory. Gives an alert prompt if dataset could not be loaded
     */
    ...
    /**
     * Creates a JFrame that is centered on screen, starts on top of all windows
     * with focus, and has no borders. This is usefull for dialogs or jfile
     * choosers, because they show on top of other windows and get the attention
     * of the user. The frame should be disposed when it is no longer useful.
     *
     * @return
     */
